...
% Task pushed in the queue must be processed, i.e., removed from the queue before the ExecutorDispatcher declares that there are no more tasks for processing.
% [ true * .
% synch_call ( 1 , ExecutorQueues , queues , pushTask ( JobPath ,taskId, false ) ) .
% true * .
% ! (synch_call ( 1 , ExecutorQueues , queues , popTask ( [ JobPath ] ) ) ) * .
% synch_eply ( 1 , ExecutorDispatcher , eDispatch , sendTaskToExecutor_return (OK, 0 ) ) ] false
% ← I thought I could fit this one with two events and an exceptional one in between;
% event1 = pushTask
% event2 = noMoreTasks 
% exceptional must happen in-between: popTask
% but the pushTask and noMoreTasks are not really related.
% ← this is a Response pattern with scope BEFORE. (S responds to P)
% [(not R)*. P. (not (S or R))*. R] false
% for paper: we actually constructed the property wrong (for case study):
% conclusion 2: the formula above does not actually repsesent what the property
% should check. !(popTask..) should be !(popTask or sendTaskToExecutor_return…)
% and ([(not R)*) at the beginning , not true*. We’ve probably devised a weaker
% property and had spurious counter-examples.
% 
...
% [true*. P. (not S)*. R] false
% 
% write the monitor, visualize it again?
