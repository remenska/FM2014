...
To describe a sequence of actions $\rho$, the concatenation $\rho\cdot\rho$ and iteration $\rho^*$ (infinite sequences) operators can be used.
...
% 
% The ability to use actions parameterized with data in mCRL2 is reflected in the
% property language as well.
% and universal and existential quantification over (possibly infinite) data types
% are allowed. For example, one may write true to denote the set of all actions,
% or exists n:Nat.val( n > 5 )&& s(n) to denote the set of s(n) actions, where
% n>5. The property [ true*. exists n: Nat.val( n > 5 ) && s(n) ]false then
% expresses that such an action never occurs.
 

here: φ represents a property, ρ represents a set of sequences of actions and α
represents a set of actions.  The formula φ1 ∧ φ2 is valid wherever both φ1 and
φ2 hold. φ1 ∨ φ2 is valid whenever φ1 or φ2 holds. The formula [ρ]φ describes
that φ holds in all states that can be reached by a sequence in ρ, while <ρ> φ
describes that φ holds in some state that can be reached by a sequence in ρ. To
describe a sequence of actions ρ, operators for concatenation (ρ.ρ) and
iteration (ρ∗) are available. The connectives ∪ and ∩ in set of actions denote
intersection and union of sets of actions. The notation !α denotes the
complement of the set of actions α with respect to the set of all actions. An
action a parameterized with data element d is denoted by a(d).


Verification using $\mu$-calculus often requires too much overhead to serve as a basis for lightweight 
